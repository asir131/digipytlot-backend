...
export async function rotateRefreshToken(oldToken: string, userAgent?: string, ip?: string) {
  ...
  if (!tokenDoc || tokenDoc.revokedAt) throw unauthorized("Refresh token revoked");
  if (tokenDoc.expiresAt < new Date()) throw unauthorized("Refresh token expired");

  const user = await User.findById(payload.sub);
  if (!user) throw unauthorized("User not found");
  if (user.status !== "active") throw unauthorized("Account not active");
  if (user.tokenVersion !== payload.tokenVersion) throw unauthorized("Session expired");
...
export async function revokeRefreshToken(token: string) {
  try {
    ...
    await RefreshToken.findOneAndUpdate(
      { tokenId: payload.jti, userId: payload.sub },
      { revokedAt: new Date() }
    );
  } catch {
